...
import { type PostgresStats, restartPlugin } from '../../lib/api.ts';
import { formatBytes, groupDigits } from '../../lib/format.ts';
import { navigate } from '../../lib/router.ts';
import { usePostgresStats } from '../../lib/useChain.ts';
...
export const PostgresView = ({ row, pluginKey, endpoint, refresh, chain }: PluginViewProps) => {
	...
	// Postgres wire-protocol stats from the control plane, keyed per endpoint+network.
	const statsQuery = usePostgresStats(endpoint, chain.network);
	const stats: PostgresStats | null = statsQuery.data?.[0] ?? null;
	const loading = statsQuery.isLoading;
	const loadErr = statsQuery.isError
			...
			{loadErr ? (
				...
				<>
					...
					>
						...
						<Kpi label="Connections" value={stats.connectionCount} token="cyan" icon="plug" />
					</div>
