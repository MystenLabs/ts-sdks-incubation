 ...
 * fetchKeys → SealClient.decrypt round trip. Per `notes/friction.md` the
 * walrus-deploy is non-idempotent across re-ups, so this spec assumes a
 * one-shot bring-up cycle (CI: `localnet:up` once, run e2e, `devstack down --purge`).
 */
...
test('alice encrypts + uploads, grants bob a cap, bob decrypts', async ({ page }) => {
	...
	await selectAccount(grantCard.getByLabel(/recipient/i), 'bob');
