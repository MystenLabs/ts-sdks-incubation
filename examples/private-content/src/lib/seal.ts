 ...
 * if the deployment changes (e.g., a fresh `localnet:up` registers a new
 * KeyServer object id).
 */
export function getSealClient(suiClient: SealCompatibleClient): SealClient {
	if (!deployment.seal) {
		throw new Error(
			'getSealClient: manifest.seal is missing. Did `devstack up` complete the seal bootstrap step?',
		);
	}
	...
	cachedClient = new SealClient({
		suiClient,
		serverConfigs: [{ objectId: deployment.seal.keyServerObjectId, weight: 1 }],
