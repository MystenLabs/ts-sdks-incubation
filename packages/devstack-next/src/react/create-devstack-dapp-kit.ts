// One-line dapp-kit setup for devstack-next-backed apps. Replaces the
...
/**
 * Build MVR-style package overrides from the devstack-next manifest.
	...
	if (localnetRpcUrl === undefined) {
		throw new Error(
			'localnetDappKitConfig: no localnetRpcUrl provided and no `sui-rpc` endpoint in manifest. ' +
				'Has `devstack-next up` reached the sui plugin?',
		);
	}
...
/**
 * Construct a dapp-kit instance wired up for devstack-next: localnet
