...
export const STATUS_USAGE = `devstack-next status [options]
...
export async function runStatus(opts: RunStatusOptions): Promise<RunStatusResult> {
	...
	if (snapshot === undefined) {
		if (opts.json === true) {
			...
			out.write(`no snapshot at ${path} — run \`devstack-next apply\` first\n`);
		}
	...
	if (opts.json === true) {
		...
		out.write(`devstack-next status → ${describeTarget(opts.env)}\n`);
