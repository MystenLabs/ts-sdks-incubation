...
Options:
  ...
  --no-tui                    Force the line-oriented plain renderer
                              (no-op for now — TUI lands in a follow-up).
...
export interface RunUpOptions {
	...
	env: Env;
	/** Where to write progress. Defaults to process.stderr. */
...
export async function runUp(opts: RunUpOptions): Promise<number> {
	...
	const detachRenderer = attachPlainRenderer(engine, { out });
	...
	const stopSignal = opts.stopSignal ?? defaultStopSignal();
...
export async function main(argv: string[]): Promise<number> {
	...
	return runUp({ config: loaded.config, env: loaded.env });
}
