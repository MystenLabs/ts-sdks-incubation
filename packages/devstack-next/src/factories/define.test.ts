...
import type { AnyNodeImpl, Dep } from '../engine/types.js';
...
describe('define', () => {
	...
	it('produces consumer Deps that carry __producer back to the source', () => {
		...
		const d = node.get('v') as Dep<unknown, unknown>;
	...
	it('attaches data when provides recipe takes a data argument', () => {
		...
		const d = node.get('lookup', { key: 'a' }) as Dep<unknown, unknown>;
	...
	it('passes through start, run, stop, restart, getStatus, inputs, represents, runsAs', () => {
		...
		expect(node.runsAs).toBe(runsAs);
	});
