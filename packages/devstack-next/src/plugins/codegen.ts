...
export interface CodegenOptions {
	...
	output?: string;
	/** Logical name. Default: `codegen.manifest`. Override when an app needs
...
// `codegen` emits a typed manifest TypeScript file under `<appDir>/<output>`.
...
export function codegen(opts: CodegenOptions = {}) {
	const output = opts.output ?? 'src/generated/manifest.ts';
	const name = opts.name ?? 'codegen.manifest';
	...
	return define<CodegenState, {}, typeof deps>({
		name,
		deps,
		inputs: ({ deps }) => ({
			output,
...
export function renderManifest(deps: ResolvedDeps): string {
	...
	return `// AUTO-GENERATED by @mysten-incubation/devstack-next codegen — do not edit.
