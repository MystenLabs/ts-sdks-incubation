...
describe('strategy-registry sibling-scope finalizer', () => {
	it.effect('drop-by-seq, not drop-by-key: closing scope A leaves sibling scope B intact', () =>
		Effect.gen(function* () {
			...
			yield* Scope.provide(registry.register(CAP_KEY, valueA, { priority: 10 }), scopeA);
			yield* Scope.provide(registry.register(CAP_KEY, valueB, { priority: 0 }), scopeB);
			...
			const afterBoth = yield* Effect.exit(registry.get<typeof CAP_KEY, StubStrategy>(CAP_KEY));
			expect(Exit.isFailure(afterBoth)).toBe(true);
		}).pipe(Effect.provide(layerStrategyRegistry)),
	);

	it.effect('symmetric: closing the LATER sibling (B) first leaves the earlier (A) intact', () =>
	...
	it.effect('finalizer runs on an INTERRUPTED scope close — no entry leaks past close', () =>
		...
		Effect.gen(function* () {
			...
			const exit = yield* Effect.exit(registry.get<typeof CAP_KEY, StubStrategy>(CAP_KEY));
