...
//
// The supervisor records the resolved absolute `deployment.json` path at
// `manifest.codegen.deploymentFile` so the read-side Vite plugin injects
// the EXACT live deployment this stack wrote as `__DEVSTACK_DEPLOYMENT__`.
...
const DEPLOYMENT_FILE = '/abs/deployment.json';

describe('manifest codegen field', () => {
	it.effect(
		'round-trips codegen.deploymentFile through write → read (decode/re-encode preserves it)',
		() =>
			withTempRoot('devstack-manifest-codegen', (tmp) =>
				Effect.gen(function* () {
					...
					const envelope = yield* buildEnvelope({
						identity: IDENTITY,
						codegen: { deploymentFile: DEPLOYMENT_FILE },
					});
					// buildEnvelope carries the field into the in-memory envelope.
					expect(envelope.codegen).toEqual({ deploymentFile: DEPLOYMENT_FILE });
					...
					expect(decoded.codegen).toEqual({ deploymentFile: DEPLOYMENT_FILE });
