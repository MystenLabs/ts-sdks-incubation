...
describe('deepbook + sui.local() composes via defineDevstack', () => {
	...
	it('under-specified local mode fails fast with a DeepbookConfigError naming the missing field', () => {
		// Local DeepBook is no longer auto-synthesized: a bare
		// `deepbook({ mode: 'local' })` (no publisher / package / pools) is a
		// compile-time error AND a runtime fail-fast. The cast bypasses the
		// required-field static type so the test can exercise the
		// `resolveLocalOptions` guard; the runtime throw is the contract.
		let thrown: unknown = null;
		try {
			deepbook({ mode: 'local' } as unknown as Parameters<typeof deepbook>[0]);
		} catch (err) {
			thrown = err;
		}
