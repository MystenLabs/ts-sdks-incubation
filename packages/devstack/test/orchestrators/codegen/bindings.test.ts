	...
	it.effect('runs sui move summary through the container runtime', () =>
		Effect.gen(function* () {
			...
			try {
				...
				// A declared toolchain moves the summary CLI off the bundled pin, so
				// bindings are generated by the same sui-tools build that publishes.
				const withToolchain = yield* Effect.gen(function* () {
					const runner = yield* MoveSummaryRunnerService;
					return yield* runner.runSummary({
						packageName: 'hello',
						sourcePath,
						moveToolchain: { suiToolsRef: 'testnet-v1.80.0' },
					});
				}).pipe(
					Effect.provide(layerSuiMoveSummaryRunnerDocker),
					Effect.provideService(ContainerRuntimeService, runtime),
				);
				if (withToolchain.cleanupPath !== undefined) {
					rmSync(withToolchain.cleanupPath, { recursive: true, force: true });
				}
