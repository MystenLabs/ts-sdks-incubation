...
describe('runStack mid-run defect tee', () => {
	it(
		'awaitShutdown re-raises a plugin scope finalizer defect that surfaces after boot',
		async () => {
			...
			try {
				...
				await Effect.runPromise(
					handle.stop.pipe(Effect.timeoutOption('10 seconds')),
				);
	...
	it(
		'awaitShutdown resolves clean when no mid-run defect occurs (no regression)',
		async () => {
