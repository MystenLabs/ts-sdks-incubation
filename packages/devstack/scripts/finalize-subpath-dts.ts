...
//
// Why this script exists: see the long comment in `tsdown.config.ts` and
// `notes/friction.md` — rolldown-plugin-dts crashes on transitive postcss
// types when bundling these subpaths, so we delegate dts emission to tsc.
// tsc emits `.d.ts` with `.js` import specifiers; we rename the files and
// rewrite the specifiers so the artefacts behave like ESM `.d.mts`.
...
const SUBPATHS = ['dapp-kit', 'vitest', 'playwright'] as const;
