...
Cross-plugin references in public plugin code use `definePlugin({ dependsOn })`; do not read them
with `ctx.get(tag)`, `ctx.use(member)`, `readConsumedTag`, or the removed `consumeMember(s)` bridge.
Those accessors and the current-engine adapter have been deleted.
