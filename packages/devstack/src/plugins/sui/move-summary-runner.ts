...
/** Outcome of checking the host `sui` CLI against the stack's toolchain. */
export type HostCliPinCheck =
	| { readonly outcome: 'ok' }
	| { readonly outcome: 'unverifiable'; readonly pinned: string }
	| {
...
/** Pure: does the host `sui` (its `--version` output) honour the toolchain
 *  the stack declared? Devstack's own default pin is never checked — the
 *  host CLI is the intended toolchain for stack-free codegen then. An
 *  explicit release-tag ref (`testnet-v1.80.0`, `mainnet-v1.78.1`) is
 *  verified against the CLI's semver. A SHA or channel-tag ref cannot be
 *  read off the version string reliably (Homebrew builds print
 *  `sui 1.77.2-homebrew`, no commit), so those are `unverifiable`. */
export const checkHostCliAgainstPin = (
	hostVersionOutput: string,
	...
	if (toolchain.kind === 'image') {
		return {
			...
			pinned: `image ${toolchain.image.tag ?? toolchain.image.digest}`,
		};
	}
	const pinned = suiToolsImage(toolchain.suiToolsRef);
	const expected = /(?:^|-)v(\d+\.\d+\.\d+)$/.exec(toolchain.suiToolsRef)?.[1];
	const host = /^sui (\d+\.\d+\.\d+)/.exec(hostVersionOutput.trim())?.[1];
	if (expected === undefined || host === undefined) {
		return { outcome: 'unverifiable', pinned };
	}
	return expected === host
		? { outcome: 'ok' }
		: { outcome: 'mismatch', pinned, ref: toolchain.suiToolsRef, expected, host };
};

/** `sui --version` on the host, or `''` when it cannot be run — the gate
 *  below treats an unreadable version as an unverifiable pin. Wrap in
 *  `Effect.cached` so one probe serves every package in an emit cycle. */
export const hostSuiVersion = (spawner: Parameters<typeof capture>[0]): Effect.Effect<string> =>
	capture(spawner, ChildProcess.make('sui', ['--version']), {
		op: 'sui --version',
		nonZeroIsFailure: true,
		stdoutTruncate: 1_000,
		stderrTruncate: 1_000,
	}).pipe(
		Effect.map((result) => result.stdout),
		Effect.orElseSucceed(() => ''),
	);

/** The host runner runs whatever `sui` is on PATH. When the stack pins an
 *  explicit toolchain, read the (memoized) `sui --version` probe and refuse
 *  a CLI that is verifiably a different release — bindings would otherwise
 *  be generated by one version and published by another. A pin that cannot
 *  be verified only warns; the default pin never probes. */
export const ensureHostCliHonoursPin = (
	probeVersion: Effect.Effect<string>,
	toolchain: MoveToolchain | undefined,
	...
	Effect.gen(function* () {
		...
		const versionOutput = yield* probeVersion;
		const check = checkHostCliAgainstPin(versionOutput, toolchain);
		switch (check.outcome) {
			case 'ok':
				return;
			case 'unverifiable':
				yield* Effect.logWarning(
					'codegen: running `sui move summary` with the host `sui` CLI ' +
						`(${versionOutput.trim() || 'version unknown'}), but the stack pins its toolchain to ` +
						`${check.pinned}, which cannot be verified against it. Bindings may differ from ` +
						'what the stack publishes with; install a matching CLI or run codegen through a ' +
						'stack boot.',
				);
				return;
			case 'mismatch':
				return yield* Effect.fail(
					new CodegenBindingsFailed({
						...
						hint:
							`The host \`sui\` CLI is ${check.host} but the stack pins its toolchain to ` +
							`${check.pinned} (${check.expected}). Bindings must be generated by the CLI that ` +
							`publishes: install it (e.g. \`suiup install sui@${check.ref}\`) or ` +
							'run codegen through a stack boot, which uses the pinned image.',
					}),
				);
		}
	});
...
> = Layer.effect(
	...
	Effect.gen(function* () {
		...
		// One `sui --version` per runner, not per package: every package in a
		// cycle shares the stack's single toolchain.
		const probeVersion = yield* Effect.cached(hostSuiVersion(spawner));
		return MoveSummaryRunnerService.of({
			runSummary: ({ packageName, sourcePath, moveToolchain }) =>
				Effect.gen(function* () {
					yield* ensureHostCliHonoursPin(probeVersion, moveToolchain, { packageName, sourcePath });
