 ...
 *  explicit toolchain, probe `sui --version` and refuse a CLI that is
...
const ensureHostCliHonoursPin = (
	...
	Effect.gen(function* () {
		...
		const versionOutput = yield* capture(spawner, ChildProcess.make('sui', ['--version']), {
			op: 'sui --version',
			...
			// An unreadable version is the same as an unverifiable pin below.
	...
	Effect.gen(function* () {
		...
		return MoveSummaryRunnerService.of({
			runSummary: ({ packageName, sourcePath, moveToolchain }) =>
				Effect.gen(function* () {
					yield* ensureHostCliHonoursPin(spawner, moveToolchain, { packageName, sourcePath });
