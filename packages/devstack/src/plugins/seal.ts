...
export interface SealOptions {
	 ...
	 * `Dep<…>` to accept either the parameterized port allocator
	 * Dep (`{slot}`) or a plain dockerContainer hostPort Dep. */
	hostPortDep?: Dep<number>;
...
export function renderSealKeyServerConfig(opts: {
	...
	return [
		'# Generated by devstack seal plugin.',
...
// `Dep<Keypair>` (TData covariance: parameterized + no-data both
// satisfy contravariant TData). `Dep<…>` rejected name-keyed
// signer Deps (`pool.get('signer', { name })` returns
// `Dep<…>`).
type SignerDep = Dep<Keypair>;
