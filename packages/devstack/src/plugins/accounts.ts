...
function signerFor(state: AccountsState, name: string): Ed25519Keypair {
	const entry = state.signers[name];
	if (entry === undefined) {
		throw new Error(`accounts.pool: signer '${name}' not in pool`);
	}
	return Ed25519Keypair.fromSecretKey(entry.secretKey);
}

const provides = {
	signer: dep((s: AccountsState, d: { name: string }) => signerFor(s, d.name)),
	// Same projection as `signer`, but the engine treats consumers of
	// `exclusive` as serialization-bound by signer name — see §3.5 in
	// the Stage 2 plan. Two `runTransaction`s declaring
	// `pool.get('exclusive', { name: 'publisher' })` won't run in the
	// same parallel batch; gas-coin contention is impossible by
	// construction.
	exclusive: exclusiveDep({
		get: (s: AccountsState, d: { name: string }) => signerFor(s, d.name),
		lockKey: (_s, d) => `accounts.signer:${d.name}`,
	}),
