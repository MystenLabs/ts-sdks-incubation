...
describe('walrusSeedWal (graph composition — no real chain)', () => {
		...
		expect(() =>
			walrusSeedWal({
				...
				accounts: [
					{
						name: 'alice',
						signer: s1.get('signer') as unknown as Dep<Ed25519Keypair>,
					},
					{
						name: 'alice',
						signer: s2.get('signer') as unknown as Dep<Ed25519Keypair>,
					},
				],
			}),
	...
	it('rejects non-positive paymentMist', () => {
		...
		expect(() =>
			walrusSeedWal({
				exchange: w.exchange!,
				accounts: [{ name: 'a', signer: s.get('signer') as unknown as Dep<Ed25519Keypair> }],
				paymentMist: 0n,
			}),
	...
	it('one runTransaction producer per account, named tx.walrus.seedWal.<name>', () => {
		...
		const seeds = walrusSeedWal({
			...
			accounts: [
				{
					name: 'publisher',
					signer: sP.get('signer') as unknown as Dep<Ed25519Keypair>,
				},
				{
					name: 'minter',
					signer: sM.get('signer') as unknown as Dep<Ed25519Keypair>,
				},
			],
		});
	...
	it('producer name encodes the account (one engine node per account)', () => {
		const w = walrus({ nodeCount: 1 });
		...
		const [seed] = walrusSeedWal({
			...
			accounts: [
				{
					name: 'publisher',
					signer: s.get('signer') as unknown as Dep<Ed25519Keypair>,
				},
			],
		});
		// Per-account serialization flows through the signer Dep's
		// `exclusiveDep` lockKey (Phase 5 design) — the producer's
		// distinct name is what makes each account its own node.
		// walrusSeedWal builds on `runTransaction` so the `tx.` prefix
		// is applied automatically.
		expect(seed!.name).toBe('tx.walrus.seedWal.publisher');
	});
