...
type CoinTypeDep = Dep<string> | Dep<{ type: string }>;
...
export function deepbookLocalnet(opts: DeepbookLocalnetOptions) {
	...
	// `start`.
	const poolDeps: Record<string, Dep<unknown>> = {};
	poolSpecs.forEach((spec, i) => {
		if (typeof spec.base !== 'string') {
			poolDeps[`pool_${i}_base`] = spec.base as Dep<unknown>;
		}
		if (typeof spec.quote !== 'string') {
			poolDeps[`pool_${i}_quote`] = spec.quote as Dep<unknown>;
		}
	});
