...
type ImageRef = string | Dep<string>;
...
// node — the rest of the cycle proceeds. Same-account serialization
// flows through `accounts.pool.get('exclusive', { name })`: the
// signer Dep's lockKey scopes the engine's parallel batch so two
// seed steps for the same account never run concurrently.
//
