...
//
// Skipped by Playwright globalSetup (which uses `setup()` →
// `engine.settle()` without entering the long-running cycle); only
...
export interface DeepbookMarketMakerOptions {
	...
	deepbookPackage: Dep<{ packageId: string }>;
...
function depositPreDeposits(args: DepositArgs): void {
	...
	for (const [coinType, amount] of totalsByCoinType) {
		// `useGasCoin: true` for SUI deposits — the SDK resolver
		// otherwise consumes every owned SUI coin as a merge-and-split
		// source for the deposit, leaving the gas-coin selector with
		// nothing and failing the tx with "No valid gas coins found".
		// Letting the gas coin double as the SUI source side-steps the
		// coin-set bookkeeping. No-op for non-SUI coins (SDK ignores
		// `useGasCoin` when `type !== '0x2::sui::SUI'`).
