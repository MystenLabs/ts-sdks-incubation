...
export interface PublishMoveOptions<TSigner> {
	 ...
	 * app's tree, or a `Dep<string>` chained off a `gitFetch(...)`
...
//     declare `signer` via `accounts.pool.get('exclusive', { name })`
//     and the engine refuses to parallel-batch publishes sharing that
//     signer (Phase 5 design; see §3.5 of the Stage 2 plan).
...
//   });
//
//   token.get('package')   // Dep<Package>
//   token.get('full')      // Dep<PublishState>
export function publishMove<TSigner>(opts: PublishMoveOptions<TSigner>) {
	...
	const deps: Record<string, Dep<unknown>> = {
		signer: opts.signer as Dep<unknown>,
		rpc: sui.get('rpc') as unknown as Dep<unknown>,
	};
	if (pathIsDep) {
		deps._path = opts.path as Dep<unknown>;
	}
...
function isPathDep(path: string | Dep<string>): path is Dep<string> {
