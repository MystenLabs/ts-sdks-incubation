// ChainOperation — typed seam for artifact publisher `produce` bodies.
//
// Architecture §10 / STYLE_GUIDE §11 Open slot O1.
//
// Three families of `produce` body exist across the plugin set; the
// artifact publisher `spec.produce: Effect<Produced, ArtifactPublishError, Scope>` is
...
// one into the `Effect<Produced, ArtifactPublishError, Scope>` shape
// `ArtifactSpec.produce` already accepts — so plugin authors
...
export type OneShotRunner = (spec: OneShotSpec) => Effect.Effect<string, ArtifactPublishError>;
...
export type ChainOperation<Produced> =
	| {
			...
			readonly executor: (
				...
				build: SuiTxBuilder,
			) => Effect.Effect<SuiEffects, ArtifactPublishError, Scope.Scope>;
			readonly parse: (effects: SuiEffects) => Effect.Effect<Produced, ArtifactPublishError>;
	  }
	| {
			readonly _tag: 'shell-oneshot';
			readonly spec: OneShotSpec;
			readonly runner: OneShotRunner;
			readonly parse: (stdout: string) => Effect.Effect<Produced, ArtifactPublishError>;
	  }
	| {
			readonly _tag: 'register-only';
			readonly produced: Effect.Effect<Produced, ArtifactPublishError, Scope.Scope>;
	  };

// ---------------------------------------------------------------------------
// Compile to an artifact publisher-spec-compatible Effect
// ---------------------------------------------------------------------------

/**
 * Compile a `ChainOperation<Produced>` into the
 * `Effect<Produced, ArtifactPublishError, Scope>` shape that
 * `ArtifactSpec.produce` accepts.
 *
	...
	switch (op._tag) {
		case 'sui-tx':
			return Effect.gen(function* () {
				...
				Effect.withSpan('substrate.artifactPublisher.produce.sui-tx', {
					attributes: { 'artifactPublisher.produce.variant': 'sui-tx' },
				}),
			);
		case 'shell-oneshot':
			return Effect.gen(function* () {
				...
				Effect.withSpan('substrate.artifactPublisher.produce.shell-oneshot', {
					attributes: {
						'artifactPublisher.produce.variant': 'shell-oneshot',
						'artifactPublisher.produce.image': op.spec.image,
					},
				}),
			);
		case 'register-only':
			return op.produced.pipe(
				Effect.withSpan('substrate.artifactPublisher.produce.register-only', {
					attributes: { 'artifactPublisher.produce.variant': 'register-only' },
				}),
			);
	}
};
