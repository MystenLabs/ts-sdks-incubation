...
export const DeepbookIndexer = <const Name extends string = 'deepbook-indexer'>(
	...
	const composite = tag(
		...
		),
		{
			...
			// Yields postgres, sui, deepbook, optional margin, and
			// iterates `dependsOn`. Lift them all into upstreams so the
			// topo scheduler places the indexer strictly after its
			// providers.
			upstreamKeys: [
				opts.postgres,
				opts.sui,
