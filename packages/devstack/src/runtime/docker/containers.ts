// Container lifecycle primitives — run, inspect, remove, stop, start,
// wait-for-healthy, wait-for-exit, read-file. Pure-data helpers (the
// argv builder for `docker run`) sit alongside so the port-bind, env,
// label, and healthcheck shape is testable without spawning docker.
//
// `node:child_process` is imported lazily inside `readContainerFile`
// so this module's static surface is browser-safe — see the comment
// in `./run.ts` for why that matters for `examples/*` Vite builds.
...
export interface ContainerInfo {
	 ...
	 * (e.g. `mysten-devstack/sui-localnet:devnet-v1.71.0-r8`). Used to
...
export interface RunContainerOptions {
	 ...
	 * wallet-app). The wallet-app has its own `host:` knob for the
