	...
	it.effect('composeStackLayer builds the full graph and resolves Sui + account', () =>
		Effect.gen(function* () {
			...
			try {
				...
				expect(sui.rpc.host).toBe('http://integration.sui.devstack.localhost:9000');
				...
				expect(sui.rpc.containerNetworks?.[0]).toMatch(
					/^devstack-integration-sui-network$/,
				);
				...
				// is the package-name-derived value (here `devstack`)
				...
				const localnetName = 'devstack-integration-sui-localnet';
				...
				expect(yamlNames).toContain('devstack-integration-sui-rpc.yml');
				expect(yamlNames).toContain('devstack-integration-sui-faucet.yml');
				expect(yamlNames).toContain('devstack-integration-sui-graphql.yml');
				// Each YAML carries the synthetic IP from the mock spawner.
				const rpcYaml = readFileSync(
					join(routerDir, 'devstack-integration-sui-rpc.yml'),
					'utf8',
				);
				...
				expect(
					localnetRun?.args.some(
						(a) => a === 'com.docker.compose.project=devstack-integration',
					),
				).toBe(true);
				...
				const pgName = 'devstack-integration-sui-indexer-db';
	...
	it.effect('layer feeds an Effect.provide consumer (the provideDevstack shape)', () =>
		Effect.gen(function* () {
			...
			try {
				...
				expect(
					recorder.some((r) => r.args.includes('devstack-integration-provide-sui-localnet')),
				).toBe(true);
			} finally {
				restore();
				yield* Effect.promise(() => rm(tmpDir, { recursive: true, force: true }));
