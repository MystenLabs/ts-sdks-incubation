...
describe('Engine — settle', () => {
	it('runs one cycle and returns when no reruns are pending', async () => {
		const a = mockProducer({ name: 'a', start: async () => 1 });
		const engine = new Engine({ stack: [a] }, { env });
		const results = await engine.settle();
		expect(results).toHaveLength(1);
		expect(results[0]!.ran.map((r) => r.name)).toEqual(['a']);
	});

	it('drains a two-cycle cascade (rerun requested in cycle 1 fires in cycle 2)', async () => {
		let rerunOnce = true;
		const startFn = vi.fn(async ({ requestRerun }) => {
			if (rerunOnce) {
				rerunOnce = false;
				requestRerun('cascade');
			}
			return 'state';
		});
		const a = mockProducer({ name: 'a', start: startFn });
	...
	it('errors cleanly at maxCycles when a producer flaps (requestRerun every cycle)', async () => {
		const startFn = vi.fn(async ({ requestRerun }) => {
			requestRerun('forever');
			return 'state';
		});
		const a = mockProducer({ name: 'a', start: startFn });
		const engine = new Engine({ stack: [a] }, { env });
		await expect(engine.settle({ maxCycles: 3 })).rejects.toThrow(/did not converge/);
		expect(startFn).toHaveBeenCalledTimes(3);
	});

	it('stops early when a cycle errors and no further reruns are pending', async () => {
		const startFn = vi.fn(async () => {
		...
		const results = await engine.settle();
		expect(results).toHaveLength(1);
		expect(results[0]!.errored.map((e) => e.name)).toEqual(['a']);
	});

	it('honors a custom maxCycles ceiling', async () => {
		const startFn = vi.fn(async ({ requestRerun }) => {
			requestRerun('flap');
			return 'state';
		});
		const a = mockProducer({ name: 'a', start: startFn });
		const engine = new Engine({ stack: [a] }, { env });
		await expect(engine.settle({ maxCycles: 7 })).rejects.toThrow(/did not converge/);
		expect(startFn).toHaveBeenCalledTimes(7);
	});
});
