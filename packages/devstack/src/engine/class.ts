...
export class Engine {
	...
	// Run cycles until no pending reruns remain (i.e. the graph has
	// settled), bounded by `maxCycles`. Cold-cold starts that need a
	// cascade — publish → bindings → manifest, say — finish in one
	// `settle()` call rather than the caller having to loop `runOnce`
	// themselves.
	//
	// Each result in the returned array corresponds to one underlying
	// cycle, in order. Cycle errors are surfaced via the standard
	// `CycleResult.errored` channel; they don't halt the loop, but they
	// also don't generate reruns, so settle naturally terminates when
	// the work runs out.
	//
	// If `maxCycles` is hit before convergence (a producer flaps by
	// calling `requestRerun` every cycle), throws — the caller should
	// see this as a producer bug, not a transient condition.
	async settle(opts?: { maxCycles?: number }): Promise<CycleResult[]> {
		const maxCycles = opts?.maxCycles ?? 8;
		if (maxCycles < 1) {
			throw new Error(`Engine.settle: maxCycles must be >= 1 (got ${maxCycles})`);
		}
		const results: CycleResult[] = [];
		for (let i = 0; i < maxCycles; i++) {
			results.push(await this.cycle());
			if (this.pendingForceRun.size === 0) return results;
		}
		const pending = [...this.pendingForceRun.keys()].join(', ');
		throw new Error(
			`Engine.settle: did not converge after ${maxCycles} cycles ` +
				`(pending reruns: ${pending || '<none>'}). ` +
				`Check producers that call requestRerun() on every cycle.`,
		);
	}
