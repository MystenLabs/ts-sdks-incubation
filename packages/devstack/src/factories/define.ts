...
>(config: DefineInput<TState, TProvides, TDeps>): Producer<TState, TProvides> {
	...
	producer.get = buildProducerGet(producer, provides) as unknown as ProducerGet<TState, TProvides>;
	return producer;
}
...
//
// Returns `Dep<never>` — the brand is bottom because the runtime
// builder doesn't know which key (and therefore which TConsumerView)
// the caller will ask for. The outer `ProducerGet<TState, TProvides>`
// cast in `define()` narrows to `Dep<View>` per key.
export function buildProducerGet(
	producer: Producer<any, any>,
	provides: Record<string, DepRecipe<any, any, any>>,
): (key: string, data?: unknown) => Dep<never> {
	return (key, data) => {
