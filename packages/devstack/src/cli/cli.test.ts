...
beforeEach(async () => {
	appDir = await mkdtemp(join(tmpdir(), 'devstack-cli-'));
...
describe('runApply', () => {
	...
	it('parallel runApply against the same stack: second sees StackLockBusyError', async () => {
		// Slow-start producer so the first runApply holds the lock for a
		// generous window — long enough that the second's acquire is
		// guaranteed to be racing even under heavy parallel test load.
		const slow = define({
			name: 'slow',
			start: async () => {
		...
		const config = defineDevstackConfig({ stack: [slow] });
		const results = await Promise.allSettled([
			runApply({ config, env, out: asWriteStream(new CaptureStream()) }),
			runApply({ config, env, out: asWriteStream(new CaptureStream()) }),
		]);
		...
		expect(err.name).toBe('StackLockBusyError');
		expect(err.message).toMatch(/locked by pid \d+/);
		expect(err.message).toContain('.devstack/networks/testnet.lock');
	});

	it('cold-cold start drains a cascade in a single runApply (settle, not runOnce)', async () => {
		// Cycle-1 producer requests a rerun once; settle should run a
		// second cycle in the same runApply invocation.
		let fired = false;
		const cascading = define({
			name: 'cascading',
			start: async ({ requestRerun }) => {
				if (!fired) {
					fired = true;
					requestRerun('cold-cold');
				}
				return 'state';
			},
		});
		const config = defineDevstackConfig({ stack: [cascading] });
		const stdoutCapture = new CaptureStream();
		const result = await runApply({
			config,
			env,
		...
		expect(result.exitCode).toBe(0);
		const summary = JSON.parse(stdoutCapture.text);
		expect(summary.cyclesRan).toBeGreaterThanOrEqual(2);
	});
});
...
describe('runWipe', () => {
	...
	it('removes the per-stack state directory', async () => {
		...
		const out = new CaptureStream();
		const result = await runWipe({ env, out: asWriteStream(out), noStop: true });
		expect(result.exitCode).toBe(0);
	...
	it('skips already-dead processes / nonexistent containers without error', async () => {
		...
		const out = new CaptureStream();
		const result = await runWipe({ env, out: asWriteStream(out) });
		expect(result.exitCode).toBe(0);
	...
	it('emits structured JSON when --json is set', async () => {
		...
		await writeSnapshot(env, snapshotWithRunnerStates(env, 0x7fff_ffff));
		const out = new CaptureStream();
		await runWipe({ env, out: asWriteStream(out), json: true, noStop: true });
		const parsed = JSON.parse(out.text) as {
			command: string;
			killed: unknown[];
			removedPaths: string[];
