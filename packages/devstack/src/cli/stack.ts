...
export const STACK_USAGE = `devstack stack <subcommand> [options]
...
Subcommands:
  ...
  down [<name>]           Stop the stack's recorded runners but keep
                          ...
                          Sister to \`drop\`, which clears state. The
                          positional defaults to --stack flag (or
                          'main' if neither is given).
  drop <name>             Wipe the named stack (= \`wipe --stack <name>\`).
                          Requires \`--yes\`.

Options:
  --yes                   Required for \`drop\`. Confirms destruction.
...
Examples:
  devstack stack list
  devstack stack list --json | jq '.stacks[] | select(.running > 0)'
  devstack stack down test
  devstack stack drop test --yes
`;
...
export async function runStackList(opts: RunStackListOptions): Promise<RunStackListResult> {
	...
	if (stacks.length === 0) {
		out.write(`no stacks for app '${opts.env.appName}' yet — run \`devstack up\` first\n`);
		return { exitCode: 0, stacks };
	}
...
export async function runStackDown(opts: RunStackDownOptions): Promise<RunStackDownResult> {
	return withStackLock(opts.env, () => runStackDownLocked(opts));
}

async function runStackDownLocked(opts: RunStackDownOptions): Promise<RunStackDownResult> {
	const out = opts.out ?? process.stdout;
	...
	if (opts.json === true) {
		...
		out.write(`(state kept; run \`devstack up\` to resume, or \`reset\` to clear)\n`);
	}
	return { exitCode: 0, killed };
}
...
//
// Locks the target stack (the one being used) — not the current env's
// stack — because the write is to the target's dir.
export async function runStackUse(opts: RunStackUseOptions): Promise<RunStackUseResult> {
	...
	const targetEnv: Env = { ...opts.env, stack: opts.name };
	return withStackLock(targetEnv, () => runStackUseLocked(opts));
}

async function runStackUseLocked(opts: RunStackUseOptions): Promise<RunStackUseResult> {
	const out = opts.out ?? process.stdout;
...
export async function main(argv: string[]): Promise<number> {
	...
	if (sub === undefined) {
		process.stderr.write(`devstack stack: subcommand required\n${STACK_USAGE}`);
		return 1;
	}
	...
	switch (sub) {
		...
		case 'new': {
			...
			if (name === undefined) {
				process.stderr.write(`devstack stack new: <name> required\n`);
				return 1;
			}
		...
		case 'use': {
			const name = positionals[1];
			if (name === undefined) {
				process.stderr.write(`devstack stack use: <name> required\n`);
				return 1;
			}
		...
		case 'drop': {
			// `stack drop <name>` delegates to `wipe` — same effect as
			// `devstack wipe --stack <name> --yes`. Confirmation flag
			// is required at the stack-drop layer too.
			const name = positionals[1];
			if (name === undefined) {
				process.stderr.write(`devstack stack drop: <name> required\n`);
				return 1;
			}
			if (!hasFlag(argv, '--yes')) {
			...
			const env: Env = { ...baseEnv.env, stack: name };
			const { runWipe } = await import('./wipe.js');
		...
		default:
			process.stderr.write(`devstack stack: unknown subcommand '${sub}'\n${STACK_USAGE}`);
			return 1;
	}
}
