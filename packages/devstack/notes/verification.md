...
- **Three sequential `apply` runs healthy on arena.** Proves state
  hydration + the `accounts.fund` non-mutating getStatus + transient
  probe-failure tolerance work end-to-end.
