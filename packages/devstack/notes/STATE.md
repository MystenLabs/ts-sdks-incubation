...
(this session)
         ...
         7b dev-wallet adapter: two-shape (legacy + devstack)
         ...
         7b examples cutover — all 5 (+ create-devstack-app template)
            on devstack; devstack.config + deployment.ts +
...
src/cli/            L7 bin — devstack up | apply | status |
...
Plus `bin: devstack`. The TUI and the CLI's programmatic exports
...
- `packages/devstack/PLAN.md` (especially L6 ~962–1127, L7 ~1128–1413)
- `packages/devstack/src/engine/class.ts` — Engine API
- `packages/devstack/src/runners/docker-container.ts` — runner pattern
- `packages/devstack/src/runners/docker-image.ts` — content-
  addressed image build runner; chains into dockerContainer via
  `image: Dep<string>` for image-aware containers
- `packages/devstack/src/plugins/sui.ts` — schema-style plugin
  composing `dockerContainer` (template for walrus / seal)
- `packages/devstack/src/plugins/accounts.ts` — disk-backed
  keystore plugin shape (define + define for Action)
- `packages/devstack/src/plugins/deepbook.ts` — simplest plugin
  shape (single static lookup)
- `packages/devstack/src/plugins/manifest.ts` +
  `packages/devstack/src/plugins/bindings.ts` — frontend
  ...
  Vite HMR.
- `packages/devstack/src/cli/apply.ts` — pattern for CLI
