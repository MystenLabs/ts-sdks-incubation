...
| Compare two `objectType` strings address-form-agnostic | `engine/sui-helpers.ts` `moveTypeEquals(actual, expected)` / `moveTypeStartsWith(objectType, prefix)` — canonicalize via `normalizeStructTag` so gRPC long-form addresses (`0x0000…0002::coin::Coin`) match JSON-RPC short-form (`0x2::coin::Coin`) without false negatives. `pickCreatedByType` uses these internally; reach for them directly when you have an `objectType` string outside an `objectChanges` walk |
...
| Content-addressed cache around a build step            | `engine/cache.ts` `withCache(spec)` — wrap any cache-keyed compute with a `verify` probe; failures of `verify` evict the cache entry and re-run. Emits a `${label}: cache hit \| miss \| verify-fail` log line per acquire so operators see cache outcomes in `pnpm devstack apply` stdout. Used by the package publish, walrus deploy, and seal keygen caches today                                                 |
...
| Boot the shared traefik router from a CLI entry        | `engine/router-bootstrap.ts` `bootstrapRouterFor(caller)` — wraps `ensureRouter` in the 10s timeout + warn-fallback + `DEVSTACK_NO_ROUTER=1` opt-out envelope used by both `devstack up` and `devstack apply`. Returns an Effect that never fails (failures log as warnings) so the caller can sequence it before its primary work without an extra catch                                                            |
