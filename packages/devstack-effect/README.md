...
Effect v4 beta reimplementation of [`@mysten-incubation/devstack`](../devstack/). Boots a hermetic
local stack (Sui localnet, Walrus, Seal, DeepBook, + your Move packages) for end-to-end dApp
development, and exposes the same services as composable Effect `Layer`s for embedding in Effect
programs.
...
```ts
import { Effect } from 'effect';
import { Sui, WalrusNetwork, SealKeyServer } from '@mysten-incubation/devstack-effect';
...
are peer deps of the consumer app, not of `devstack-effect` — install the version you need
separately.
...
`Coin.sdkCoin` (and every entry in `@mysten-incubation/devstack-effect`'s coin records) mirrors the
SDK's `Coin` shape (`{address, type, scalar}`):

```ts
import { Effect } from 'effect';
import { Sui, DeepbookCore, Coin } from '@mysten-incubation/devstack-effect';
...
`@mysten-incubation/devstack-effect` is **incubation-stage** — `private: true` in this monorepo,
