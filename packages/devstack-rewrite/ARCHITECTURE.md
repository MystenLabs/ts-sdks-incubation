# Devstack rewrite architecture guide
...
Anchor doc: `../devstack/notes/redesign/architecture.md` (2813 lines, 7 revision rounds). This guide
distills the invariants; that doc is the design source of truth.

Companion: see `STYLE_GUIDE.md` for code-level patterns. See `notes/parity-matrix.md` for the
cutover gate.
...
| **L2 plugins**                   | sui, postgres, walrus, seal, account, faucet, package, coin, wallet, action — one folder each conforming to `NodePlugin`. Renderer plugins (TUI Ink, plain, silent) also here. Per-plugin tagged errors, Snapshotable / Routable / NetworkResolver-mode / Codegenable / StrategyContributor decls. **`deepbook` slot DELETED — rebuilt fresh in PR3 against `api-surface-design.md` §8 Example 5.**                                                                                                                                                                    | L0, L1, **other plugins via substrate tags (consumes) only — NEVER direct module imports**                                                           | other plugin INTERNAL modules. Other services' source.                                                                                          |
...
| ----------------------- | ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| **NodePlugin**          | `contracts/node-plugin.ts` (re-export of `StackMember`) | Universal plugin shape: 4 generics `(Provides, Consumes, Caps, Siblings)`, `acquire` body yields services from R-channel                      | Every plugin                                                                                                                      |
...
| **Codegenable**         | `contracts/codegenable.ts`                              | Emitter contribution: `emit() → Record + outputPath + sensitive flag`                                                                         | Any plugin emitting type-safe bindings (every L2 plugin today)                                                                    |
...
| **CompositePrimitive**  | `contracts/composite-primitive.ts`                      | Composite-plugin first-class contract: one row, many children (walrus 4-shard cluster + aggregator + publisher)                               | Composite plugins only                                                                                                            |
...
- `Renderer` (`contracts/renderer.ts`) — subscribable-state-driven; substrate-of-surfaces. Pluggable
  rendering.
- `LifenessClassifier` (`contracts/liveness-classifier.ts`) — narrow contract for one-shot vs
...
- `defineNodePlugin({ provides, consumes, kind, acquire, capabilities })` is the explicit authoring
  API (`api/define-plugin.ts`).
- All capability decls are reachable from a public `contracts/` import.
- The `defineModeNamespace` + `forNetwork` API is available to custom plugins for mode-narrowed
  factories.
...
## R-channel widening (Round 7 of architecture)

Plugin `acquire` bodies yield substrate services from `Effect.gen`'s R-channel:
...
Cross-plugin references in `acquire` bodies use either `ctx.get(tag)` (when the consumes tuple is
concrete) or `ctx.use(member)` (when the outer plugin's `Consumes` generic carries the tuple — the
case where `get(tag)`'s `T extends Provided` constraint widens to the substrate-erased `AnyTag`).
The `use(member)` accessor preserves the member's literal-typed `provides` generic; a member whose
tag id is not in `Consumes` surfaces a branded `__MemberNotConsumedError<Id>` at the call site. The
three plugins (`wallet`, `coin`, `package`) that currently cast-as-escape-hatch via
`(ctx as { get: (t: typeof Tag) => Resolved }).get(Tag)` will migrate to `ctx.use(member)`.
...
Apps (dapp-kit, user code shipping to production) MUST NOT import from `@mysten-incubation/devstack`
(or `-rewrite` pre-cutover).
...
| BuildContext (`get(tag)` + `use(member)`)                       | `substrate/plugin.ts` (type) + `substrate/runtime/lifecycle/plugin-registry.ts` (impl)                                            | OK                  | Direct member-ref accessor preserves the literal-typed `provides` generic; branded `__MemberNotConsumedError<Id>` at the call site for non-consumed members.                                                                                                                                                  |
...
| Observability — `Logger`                                        | `substrate/runtime/observability/logger.ts`                                                                                       | IN-FLIGHT (PR2-A)   | Zero call sites today; PR2-A wires via the harvest loop.                                                                                                                                                                                                                                                      |
| Observability — `SpanAttr`                                      | `substrate/runtime/observability/spans.ts`                                                                                        | IN-FLIGHT (PR2-A)   | Defined but only referenced by `spans.ts`; PR2-A retires free-form keys.                                                                                                                                                                                                                                      |
...
- O8: `runtime.ensureManagedContainer` helper.
...
L2 plugins           — sui, postgres, walrus, seal, account, faucet,
                       ...
                       go through substrate tags via `consumes`.
...
The rewrite cannot ship while these CLOSE rows are open. WIN / PARITY / ACCEPT_GAP rows are fine.
...
Specific rewrite findings come from:
