...
- Vitest tests live in `packages/devstack-rewrite/test/` mirroring the `src/` directory structure.
...
- Known bad pattern (see `notes/api-comparison.md` opportunity): every rewrite example config
  carries 10-30 lines of "Differences from v3" header comments. **Strip at cutover.** New examples
  must not adopt this style.
...
- Cross-package imports inside this monorepo use `@mysten-incubation/devstack-rewrite/<subpath>`;
  relative `.ts` imports inside the package use the explicit `.ts` extension (the rewrite package
...
- Plugin barrel (`plugins/<name>/index.ts`) re-exports only what is **user-public**. Plugin
  internals (mode files, registries, errors taxonomy) are reachable via
  `import { ... } from '@mysten-incubation/devstack-rewrite/plugins/<name>'` only via what the
  barrel chooses.
...
export class FooRegistryService extends Context.Service<FooRegistryService, FooRegistry>()(
	'@devstack-rewrite/plugins/foo/FooRegistry',
) {}
