// Cross-cutting composite-plugin errors.
//
...
// generic ("which composite variant refused" + "which network it
// refused under") and do NOT name walrus / seal.

import { Schema } from 'effect';

/**
 * Synchronous factory-time refusal raised by a composite-plugin
 ...
 * cross-plugin composition through resolved values).
 *
 * Used by walrus (`local-cluster` variant) and seal (`local-keygen`
 * variant) today; future composite plugins refusing on a network
...
export class ForkIncompatibleError extends Schema.TaggedErrorClass<ForkIncompatibleError>()(
	'ForkIncompatibleError',
	{
		 ...
		 *  `'sealLocalKeygen'`. Lets multiple composite plugins share
