...
(`npx @mysten-incubation/dev-wallet serve`). The endpoints are implemented as middleware that can be
mounted on any HTTP server.
...
client for this API. See [Remote CLI Adapter](/dev-wallet/reference/adapters/remote-cli) for usage.
