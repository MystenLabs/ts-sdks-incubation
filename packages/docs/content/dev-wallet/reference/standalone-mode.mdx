...
<Callout type="info">
	A copy of this wallet is hosted at `https://sui-dev-wallet.vercel.app`. Use that origin wherever
	this page shows `http://localhost:5174`. The CLI signer isn't available there. See [Hosted
	wallet](/dev-wallet/guides/hosted-wallet).
</Callout>
...
Drag the **Dev Wallet** button from the wallet page (or its Settings tab on small screens) into your
browser bookmarks bar, or paste this in your browser console:
...
The bookmarklet reads the wallet origin from its own script URL. It only stays in effect until the
page reloads.
