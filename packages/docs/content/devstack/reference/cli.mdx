...
`PATH` for the `sui move summary` step. When the stack pins an explicit `suiToolsRef`, the host CLI
is checked against it and a verifiable release mismatch fails with a remediation hint (see
[Sui config](/devstack/configure/sui#local-mode)). Writes the generated tree only.
