...
`deployment.json` and carries the resolved ids, accounts, and dev-wallet connection. Reconciles
through a live supervisor when present; otherwise boots once, emits, and exits. The committed
`src/generated` tree is `devstack codegen`'s job.
...
re-emits the per-stack deployment file, and exits. Run `devstack codegen` to refresh the committed
`src/generated` tree.
