...
Re-emit the per-stack deployment file and dev extras from a live or one-shot stack. The deployment
file is `deployment.json`. Reconciles through a live supervisor when present; otherwise boots once,
emits, and exits. The committed `src/generated` tree is `devstack codegen`'s job.
...
`PATH` for the `sui move summary` step. Writes the committed tree only.
...
re-emits the per-stack deployment file and dev extras, and exits. Run `devstack codegen` to refresh
the committed `src/generated` tree.
...
Each failure maps to a specific code. The devstack-domain block (40–43) is a private extension
carved out below the standard sysexits range so it never collides with the OS table; numbers are
stable and never reused. `devstack schema --json` emits this table for automation.
