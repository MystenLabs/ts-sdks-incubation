...
`deployment.json`; dev extras are the `generated-extras` tree. Reconciles through a live supervisor
when present; otherwise boots once, emits, and exits. Never writes the committed `src/generated`
tree — that is `devstack codegen`'s job.
...
Emit the stack deployment (deployment.json) for a real-network deploy. Writes the current (or
booted) stack's `deployment.json` to `--out` or stdout — use it to obtain a committed deployment for
the deploy (see [Codegen](/devstack/features/codegen)).
