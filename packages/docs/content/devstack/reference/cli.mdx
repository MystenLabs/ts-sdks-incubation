...
`--renderer <tui|plain|silent>`, `--from-snapshot <name-or-id>`,
...
`--from-snapshot <name-or-id>`, and `--snapshot-stale warn|block|clean-start`. The default snapshot
stale policy is `warn`.
