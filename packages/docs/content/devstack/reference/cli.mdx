...
Re-emit the per-stack ids file and dev extras from a live or one-shot stack. The ids file is
`devstack-ids.json`; dev extras are the `generated-extras` tree. Reconciles through a live
supervisor when present; otherwise boots once, emits, and exits. Never writes the committed
`src/generated` tree — that is `devstack codegen`'s job.
...
Regenerate committed bindings from Move source (no stack boot). The committed tree is
`src/generated`. Deterministic and stack-free: no Docker, no on-chain ids. Writes the committed tree
only — never the ids file.

Lifecycle: one-shot. Side effects: write. Docker: no. Options: `--json`, `--app <name>`,
`--stack <name>`, `--state-dir <path>`, `--verbose`, `--config <path>`, `--network <name>`.

### `devstack dump-ids [options]`

Emit the stack id-config (devstack-ids.json) for a real-network deploy. Writes the current (or
booted) stack's `devstack-ids.json` to `--out` or stdout — use it to obtain a committed id-config
for the deploy (see [Codegen](/devstack/features/codegen)).
...
re-emits the per-stack ids file and dev extras, and exits. `apply` does not touch the committed
`src/generated` tree — run `devstack codegen` for that.
