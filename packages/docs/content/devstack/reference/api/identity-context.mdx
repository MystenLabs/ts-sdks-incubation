...
- `app` — the `--app` CLI flag / `$DEVSTACK_APP`, else inferred from the nearest package.json `name`
  (the unscoped tail), else the default `'devstack'`.
...
- **Per-stack runtime state.** The deployment file lives under `.devstack/stacks/<stack>/` and
  ...
  and id-free; see [Codegen & deployments](/devstack/codegen).)
