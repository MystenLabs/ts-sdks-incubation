...
- **Per-stack runtime state.** The deployment file and the dev-only extras live under
  `.devstack/stacks/<stack>/`, so two stacks of the same app keep distinct on-chain ids and
  dev-wallet state. (The committed `src/generated` tree is stack-invariant and id-free; see
  [Generated Outputs](/devstack/codegen).)
