...
substrate threads it purely as a correlation/display value (supervisor labels, span attributes,
snapshot provenance) and stores it verbatim — it carries no network/mode semantics at this layer.
The CLI / `runStack` set it to the resolved network name (`'localnet'`, `'testnet'`,
`'mainnet-fork'`, …) by convention.
