...
- A runtime check verifies every element returned by the builder carries the plugin brand, so a
  value that skips `definePlugin` (e.g. cast through `as unknown as AnyPlugin`) is rejected at
  composition with a clear message.
