...
It's one config flowing through a handful of steps. The
...
You don't hand-author that file — you generate it from a real deploy in the next step, then commit
...
## 2. Generate a typed deployment from a real deploy
...
sui client test-publish move/connect_four --build-env devnet
```

Then scaffold the typed file. `dump-deployment --network <net>` does not publish — it resolves the
...
From here you edit and complete it like any source file, then commit it — the committed
