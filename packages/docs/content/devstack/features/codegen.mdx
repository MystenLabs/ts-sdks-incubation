...
Default root for the home stack:

```txt
src/generated/
```

Non-home stacks (any run with an explicit `--stack`/`$DEVSTACK_STACK` override that diverges from
...
the same location the writer chose. `pnpm dev` (home stack) and `pnpm test:e2e` (an `e2e` stack)
...
The home/non-home rule above is the default. An app can pin an explicit output dir in the stack
...
`src/generated/` before the first `up`), so an import resolves to the home stack under `pnpm dev`
and to `.devstack/stacks/<stack>/generated/` under a non-home stack — automatically, with no import
changes.
