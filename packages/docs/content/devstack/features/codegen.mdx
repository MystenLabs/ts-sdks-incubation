...
- `pnpm dev` / `devstack up` — boots the stack and writes a gitignored
  ...
  those live ids automatically in dev.
...
`devstack-ids.json` schema the boot writes, selected at build time. Obtain one by booting the stack
against the target network once and copying the file it emits, or hand-author it:
