...
endpoint, and the balance APIs (`getBalance` / `listBalances` / `getCoinInfo`) intentionally panic in
fork mode — read balances through `ChainProbe`, and fund accounts through the fork faucet (below).
