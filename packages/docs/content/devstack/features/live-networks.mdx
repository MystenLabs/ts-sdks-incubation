...
where they are hard-clamped **off**. This keeps localnet, fork, and even live testnet ergonomic for
development while ensuring a build pointed at live mainnet can never ship a dev wallet or a faucet.

`autoApproveSigning` defaults **off** on every network. A real app should exercise the actual
connect and approve flow, so `pnpm dev` shows the wallet prompts rather than silently signing. Opt
...
All three conveniences are hard-clamped **off** on live mainnet regardless of overrides — a
real-funds signature is never auto-approved, and the clamp is not opt-out: it applies whenever the
resolved surface network is live mainnet.
