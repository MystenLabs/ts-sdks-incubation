---
title: Actions
description: Declare one-shot on-chain effects (mint, create-singleton, seed-config) that run once after their dependencies are ready.
---
...
Actions are `role: 'task'`: the body runs once at acquire and reaches "done" after `start`
resolves. They are not long-lived services.
...
discriminator changes. Supplying a callback `discriminator` is how you force re-execution on
mutable upstream state (e.g. a counter on a singleton object).
