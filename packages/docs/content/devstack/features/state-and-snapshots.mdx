...
- The supported snapshot surface is the CLI and the on-disk artifact described above. Treat
  `meta.json`, `state.json`, `host-tree.tar`, `containers/images.tar`, and `integrity.json` as the
  artifact contract; everything else under the snapshot directory is implementation detail and not
  load-bearing for app or plugin-author code.
