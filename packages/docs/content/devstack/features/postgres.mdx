---
title: Postgres
description: Run a managed Postgres container alongside the stack and consume it from app code via generated bindings.
---
...
the local container exists today. The plain-endpoint shape anticipates a future live mode
(Cloud SQL / Neon / RDS) without a code-shape change at the call site.
...
- `password` — credential. If omitted, devstack derives a deterministic per-stack password from the
  identity `(app, stack, name)` so the value is stable across cold boots without leaking through
  config.
...
- `route` — when `true`, contribute a TCP routable on the `postgres-tcp` Traefik entrypoint
  (host port 5432 by default). Default `false`. Only one stack on the host may set `route: true` at
  a time — TCP entrypoints serve one backend at a time and the router collision detector fails fast
