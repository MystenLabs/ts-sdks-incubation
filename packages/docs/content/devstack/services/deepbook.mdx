...
`deepbook(...)` adds a [DeepBook](https://github.com/MystenLabs/deepbookv3) central-limit order-book
DEX to your stack. It runs in one of three modes:
...
- **`known`** — point at an existing DeepBook deployment (testnet / mainnet) by network shortcut or
  explicit ids. Nothing is published; your app reads and trades against the live DEX.
...
// SUI for the publish + pool-creation gas and the quote-side seed deposit.
...
`deepbook(...)` generates typed bindings into `src/generated/deepbook.ts`. Import them to read the
created pool ids and Pyth feed prices without hard-coding anything:

```ts title="src/market.ts"
import { deepbook } from './generated/deepbook.js';

const dex = deepbook.deepbook;
...
DeepBook is a per-network service: `deepbook.forNetwork(network).deepbook` returns the bindings for
the network your wallet is connected to, so a runtime `switchNetwork` flips the package, pool, and
coin ids in lockstep. Extend your Sui client with the DeepBook extension built from that bucket:
...
nothing hard-coded — so `deepbookClient.deepbook.deepBook.swapExactQuoteForBase(...)` and the rest of
the SDK target the pools your stack just created. See
[`examples/deepbook-trader`](https://github.com/MystenLabs/ts-sdks-incubation/tree/main/examples/deepbook-trader/src/App.tsx)
for the `buildPackageIds` / `buildCoinMap` / `buildPoolMap` helpers and a complete trading UI —
multiple pools, multiple feeds, and swap execution.
...
shortcut when available. Pyth resolves to the live network deployment automatically — you don't pass
a `pyth` block.
...
Margin, indexer, server, and market-maker options are intentionally absent until those modes have
real acquire behavior. `deepbook(...)` returns a service member; known testnet/mainnet helpers may
also contribute funding strategies for service-owned coins when the mode supports them.
