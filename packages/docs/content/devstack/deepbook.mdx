...
- **`known`** — point at an existing DeepBook deployment (testnet / mainnet). The zero-config
  ...
  `@mysten/deepbook-v3` SDK, so there's nothing to copy and nothing to drift; `.known({ packageId,
  registryId })` is the raw-id override. Nothing is published; your app reads and trades against the
  live DEX.
...
`deepbookFor(network).known({ packageId, registryId })` (or `deepbook({ mode: 'known', packageId,
registryId })`) and pass both ids explicitly.
