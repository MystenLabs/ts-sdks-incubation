...
Fork state is keyed by the binary that wrote it, so switching between a sui-tools build and the
source build (or between two sui-tools refs) seeds a fresh fork data dir rather than reusing one.

`suiToolsRef` cannot be combined with `version` (which only pins the source build) or with
`image: { pull }`. The other escape hatches still work: `image: { pull: '…' }` runs a complete
prebuilt image you maintain, and `DEVSTACK_SUI_FORK_IMAGE` does the same for a single run, falling
back to the source build if the pull fails.
