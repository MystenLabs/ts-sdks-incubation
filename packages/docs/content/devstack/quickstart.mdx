...
`devstackVitePlugin()` wires the `@generated` alias to the active stack's codegen output.
...
The generated files are importable from application code through the `@generated` alias. For
dapp-kit wiring, consume the generated config file and the generated Sui network file:
