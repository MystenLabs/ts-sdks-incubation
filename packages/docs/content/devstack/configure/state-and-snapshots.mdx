...
plain renderer startup sweeps. A missing, malformed, or unsupported file is treated as absent;
devstack rebuilds status from the next running stack.
...
- `up --snapshot-cache <name>` restores that named snapshot when current and refreshes it after
  startup when it is missing or stale.
...
- Snapshots are stack-scoped. Snapshot metadata carries app, stack, network, and plugin identity
  slices (including a Sui `mode` discriminator); restore raises `IdentityMismatchError` on identity
  drift before any destructive change.
...
- The supported snapshot surface is the CLI and the on-disk artifact described above. `meta.json`,
  `state.json`, `host-tree.tar`, and `containers/images.tar` are the artifact contract; everything
  else under the snapshot directory is implementation detail.
