...
Treat `meta.json`, `state.json`, `host-tree.tar`, and `containers/images.tar` as the stable artifact
contract; anything else under the directory is internal.

## Things to know

- Snapshots are stack-scoped — you save and restore one stack at a time.
- `restore` and `up --from-snapshot` refuse to run while `devstack up` owns the stack, and prompt
  for `y` unless you pass `--yes`.
