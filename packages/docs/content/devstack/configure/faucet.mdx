...
request through it; otherwise SUI funding raises the substrate `StrategyNotFoundError`, listing the
...
- Skipping the capability decl and calling into a faucet HTTP endpoint directly means **the registry
  ...
  `StrategyNotFoundError` even though the plugin "works". Snapshot capture also misses the
...
- Mounting with a stale chain id (e.g. hardcoded `'sui:localnet'` when the stack is in
  ...
  account funds against the wrong network.
...
A faucet strategy raises a tagged error on each failure path. The two load-bearing rules:

- A non-2xx HTTP status raises `FaucetUnreachable` or `FaucetBodyError`. The Sui faucet binary binds
  its socket before its validator can transfer coins, so the warm-up window returns 5xx; a
  permissive implementation would mark accounts funded when no coins moved.
- A 200 OK body carrying `{ status: { Failure: ... } }` raises `FaucetBodyError`. During warm-up the
  faucet accepts requests it cannot execute, so those bodies are treated as failures.
...
The [web dashboard](/devstack/configure/dashboard)'s Faucet panel exposes this same funding as a
one-click **fund** action: SUI funds a fixed amount, while WAL and DEEP take an editable amount and
fund a resolved account. It dispatches through the same registered funding strategies described
above — the boot-time account-funding pass and the dashboard button share one code path.
