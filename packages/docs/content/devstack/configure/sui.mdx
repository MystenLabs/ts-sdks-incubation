...
- **`suiToolsRef`** — the `mysten/sui-tools` tag or commit SHA the validator image is built on
  ...
  the same thing for a single run; config wins when both are present. The env var is stack-wide: it
  also rebases fork mode and the stack-free `devstack codegen` image. Not combinable with
  `image: { pull }`, which names a complete image.
