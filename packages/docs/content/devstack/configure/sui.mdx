...
- **`suiToolsRef`** — the `mysten/sui-tools` tag or commit SHA the validator image is built on
  ...
  selects the CLI that `devstack codegen` runs `sui move summary` with, so generated bindings match
  the toolchain that builds and publishes. Not combinable with `image: { pull }`, which names a
  complete image.
