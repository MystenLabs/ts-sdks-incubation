...
- **`suiToolsRef`** — the `mysten/sui-tools` tag or commit SHA the validator image is built on
  (`'testnet-v1.80.0'`, or a 40-character SHA). Omit it for devstack's bundled pin. The `-arm64`
  suffix is added for you on Apple Silicon. The `DEVSTACK_SUI_TOOLS_REF` environment variable sets
  the same thing for a single run; config wins when both are present. Either way the choice also
  selects the CLI a stack boot runs `sui move summary` with, so generated bindings match the
  toolchain that publishes. (The stack-free `devstack codegen` verb uses the host `sui` CLI and
  fails when the stack pins a release the host CLI does not match.) The config field cannot be
  combined with `image: { pull }`, which names a complete image; the env var is simply ignored when
  `image: { pull }` is set.
- **`image`** — override the validator image: `{ pull: '<ref>' }` skips the build, or
  `{ build: { context, dockerfile? } }` builds your Dockerfile with devstack's build args (including
  the resolved `SUI_TOOLS_IMAGE`) and a content-hashed tag.
...
`suiToolsRef` applies here too, and matters more: naming a sui-tools build that ships `sui-fork`
replaces a ten-minute source compile with an image pull.
