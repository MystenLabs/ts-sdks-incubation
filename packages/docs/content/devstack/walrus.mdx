...
generated Walrus binding exposes them as `publisherUrl` and `aggregatorUrl`.
...
const blobId = receipt.newlyCreated.blob_id;

const read = await fetch(`${w.aggregatorUrl}/v1/blobs/${blobId}`);
const bytes = new Uint8Array(await read.arrayBuffer());
```

The local publisher owns its own persisted dev signer under the Walrus runtime directory, funds it
from the local SUI faucet, swaps SUI for WAL through the local exchange, and pays storage costs for
uploads. The signer is an implementation detail of the local endpoint; accounts that need to own the
returned blob object can pass `send_object_to=<address>` on the publish URL and the endpoint will
transfer the blob object after writing it.
