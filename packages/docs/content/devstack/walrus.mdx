...
node list can't be derived and stays required.) Use `walrusFor(network).known({ systemObjectId,
stakingPoolId, nodes })` to override the ids for a deployment outside the SDK constants. Fork stacks
use the same `walrusFor(forkNetwork).testnet({ nodes })` methods; local Walrus clusters run only on
local networks.
