...
stack to start the cluster, then use the generated publisher and aggregator URLs to publish and read
blobs through the release Walrus services.
...
## Local publisher and aggregator

Local mode starts app-facing publisher and aggregator HTTP endpoints by default. These endpoints let
an app publish and read blobs through one URL instead of talking directly to every storage node. The
generated Walrus binding exposes them as `publisherUrl` and `aggregatorUrl`. If you disable either
service, that URL resolves to `null`.

```ts
if (!w.publisherUrl || !w.aggregatorUrl) {
	throw new Error('Walrus publisher and aggregator services are disabled for this stack');
}
...
Devstack runs the release-provided `walrus publisher` and `walrus aggregator` binaries in managed
containers. The publisher uses the local deploy-generated wallet and native Walrus publisher
behavior, including support for `send_object_to=<address>` on the publish URL.

Tune or disable the services through `local.aggregator` and `local.publisher`:
...
const storageNodesOnly = walrus({ local: { aggregator: false, publisher: false } });
```
...
For local clusters, prefer the generated `publisherUrl` and `aggregatorUrl` shown above. The local
storage-node committee records use Docker-network hostnames so the Rust publisher and aggregator
containers can reach the nodes directly; those hostnames are not a browser API. Build a direct
`WalrusClient` only for known/live deployments, or for advanced host-side tooling that deliberately
manages storage-node routing itself:
...
if (w.mode === 'local') {
	throw new Error('Use w.publisherUrl and w.aggregatorUrl for local Walrus clusters');
}
