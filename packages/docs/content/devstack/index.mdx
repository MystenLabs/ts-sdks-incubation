...
title: Get started
description:
  Boot a local Sui stack, fund accounts, publish packages, wire your app, and generate bindings —
  all from one TypeScript config.
---
...
This writes the `src/generated` tree — typed Move bindings and the runtime config your app imports —
...
2. Run `devstack codegen` to write the bindings and runtime config under `src/generated` — the tree
   your app builds against. Rerun it when the Move source changes.
...
- [Codegen & deployments](/devstack/codegen) for generated bindings and the deployment config.
