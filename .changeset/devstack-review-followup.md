...
- **Sui snapshots now carry a `mode` discriminator in their restore identity.** A container-`local` snapshot and a `local-rpc` snapshot at the same chain id are no longer mutually restorable (the cross-mode restore was a silent no-op).
